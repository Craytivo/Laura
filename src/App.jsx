...
export default function App(){
 ...
 return <div className="site"><Header open={open} onToggle={()=>setOpen(value=>!value)} onNavigate={navigate} onBook={book} business={site} service={signature}/><main>
  <Hero image={images[0]} service={signature} business={site} onBook={book} onNavigate={navigate}/><SignatureService image={images[1]} service={signature} business={site} onBook={book}/><Intro business={site}/><WhySugaring features={site.why}/><Services services={services} selected={selected} onSelect={selectService}/><Bundles bundles={bundles} onSelect={selectService}/><BookingSummary selected={selected} onBook={book} onNavigate={navigate}/><Loyalty service={signature} loyalty={site.loyalty} onSelect={selectService}/><FAQ items={site.faq}/><FinalCTA onBook={book}/></main><MobileBookingBar selected={selected} fallback={signature} onBook={book}/><Footer business={site} onNavigate={navigate}/></div>;
}
